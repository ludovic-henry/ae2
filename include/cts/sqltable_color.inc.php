 ...
 * @see stdentity
 */
class sqltable_color extends stdcontents
{
  var $id_field;
  var $id_name;
  var $page;
   ...
   **/
  function sqltable_color ( $formname, $title, $sql, $page, $id_field, $cols, $actions, $batch_actions, $enumerated=array(), $htmlentitize = true, $fjs=true, $hilight=array(), $anchor="")
  {
    ...
    while ( is_array($sql) ? (list($rien,$row) = each($sql)) : ($row = $sql->get_row()) )
    {
      ...
      if ( count($batch_actions) )
      {
        ...
        $this->buffer .= "<tr id=\"ln[0]\" class=\"$style\" onMouseDown=\"setPointer('ln$t','$num','click');\" onMouseOut=\"setPointer('$style','$num','out');\" onMouseOver=\"setPointer('ln$t','$num','over');\">\n";
